...
function ESP:AddPlayer(player, color)
    ...
    local characterAddedConnection = player.CharacterAdded:Connect(function(character)
        ...
        -- Clear cached character parts so ESP recalculates for the new character
    ...
    local characterRemovedConnection = player.CharacterRemoving:Connect(function()
        ...
        -- Clear cached character parts for this player
        if characterCache then
            characterCache[player] = nil
        end
    end)
...
function ESP:AddTeam(teamName, color, useTeamColor)
    ...
    -- Track players who join after AddTeam was called
    if not self.Connections["TeamTracker_" .. teamName] then
        self.Connections["TeamTracker_" .. teamName] = players.PlayerAdded:Connect(function(player)
            if player == localPlayer then return end
            if teamName == "Criminal" then
                ...
                self.Connections[player] = self.Connections[player] or {}
                local childAddedConn = player.ChildAdded:Connect(function(child)
                    if child and child.Name == "Is_Wanted" then
                        self:AddPlayer(player, Color3.new(1,0,0))
                    end
                end)
                local childRemovedConn = player.ChildRemoved:Connect(function(child)
...
function ESP:AddObjectListener(path, childToListenFor, color)
    local connection
    if path and typeof(childToListenFor) == "string" then
        connection = path.ChildAdded:Connect(function(child)
            if child and child.Name == childToListenFor and (child:IsA("BasePart") or child:IsA("Model")) then
                if child:IsA("Model") then
                    if not child.PrimaryPart then return end
                    child = child.PrimaryPart
                end
                if child and child.Position then
                    ESP:AddInstance(child, color)
                end
            end
        end)
    end
